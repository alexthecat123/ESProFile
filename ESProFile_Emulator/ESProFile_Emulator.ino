...
#define timeout 1000000 // Timeout value (in loop iterations) for all phases of ProFile comms

#define red 32 // Pins for the red and green LEDs
#define green 4
...
void setup(){
  ...
  Serial.begin(115200); // And serial comms too
...
void readDrive(){
  ...
  while(bitRead(REG_READ(GPIO_IN_REG), CMDPin) == 1){ // Stay in the data-sending loop until the host lowers CMD
    currentState = bitRead(REG_READ(GPIO_IN_REG), STRBPin); // Read the current state of STRB
    if(currentState == 1 and prevState == 0){ // If we see a rising edge on STRB, send the next byte of the block (the host reads on the falling edge, so we should send on the rising edge)
...
void writeDrive(){
  ...
  while(bitRead(REG_READ(GPIO_IN_REG), CMDPin) == 1){ // Stay in the status byte-sending loop until the host lowers CMD
    currentState = bitRead(REG_READ(GPIO_IN_REG), STRBPin); // Read the current state of STRB
    if(currentState == 1 and prevState == 0){ // If we see a rising edge on STRB, send the next status byte (the host reads on the falling edge, so we should send on the rising edge)
...
// Calculates the (odd) parity of a byte and sends it out over the PARITY pin
